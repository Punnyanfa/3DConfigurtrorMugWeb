...
function App() {
  ...
  const handleManufacturerChange = (e) => {
    ...
    setTotalCost(preloadedDesign?.totalCost || 0);
  };

  // Update total cost based on applied services
        ...
        img.onload = () => {
          ...
          if (textureChangeService) {
            ...
            setDesignData((prev) => ({
              ...
              textures: {
                ...prev.textures,
                [component]: { dataURL: e.target.result, settings: prev.textures[component]?.settings || {} },
              },
            }));
            updateTotalCost();
          }
  ...
  const handleDesignUpdate = (data) => {
    setDesignData((prev) => {
      ...
      Object.keys(data.textures).forEach((key) => {
        ...
        newTextures[key] = textureData ? { dataURL: textureData.dataURL || customTexture?.dataURL, settings: textureData.settings } : null;
      });
      return {
        ...prev,
        model: selectedModel,
  ...
  const handleSaveToJson = () => {
    if (!selectedManufacturer) {
      alert('Vui lòng chọn một nhà sản xuất trước khi lưu.');
      return;
    }
    saveDesign();
  };

  // New function to handle .json file download
  const handleExportJson = () => {
    if (!selectedManufacturer) {
      alert('Vui lòng chọn một nhà sản xuất trước khi xuất.');
      return;
    }
    const designToSave = {
      id: preloadedDesign?.id || Date.now(),
      timestamp: new Date().toISOString(),
      data: {
        ...designData,
        manufacturer: selectedManufacturer,
    ...
    link.href = url;
    link.download = `design_${designToSave.id}.json`;
  ...
  const handleComplete = () => {
    if (!selectedManufacturer) {
      alert('Vùi lòng chọn một nhà sản xuất trước khi hoàn thành.');
      return;
    }
    setShowPopup(true);
  };
  ...
  return (
    <div className="container">
      ...
      <div className="complete-container">
        ...
        <button className="done-btn" onClick={handleSaveToJson}>
          Done
        </button>
